      ...
      <v-container row>
        <v-row align="center">
          <v-col class="text-center" cols="12" sm="8">
            <h2>
              {{ activeName }}
              <v-btn icon color="blue" @click="checklistDialogToggle">
...
export default {
  ...
  mounted () {
    ...
    this.play(firstLoad)
    data.forEach((e) => {
      this.checklist[e.name] = localStorage[e.name]
    })
  },
  methods: {
    courseChange (name) {
      localStorage[name] = this.checklist[name]
    },
    checklistDialogToggle () {
